...
def make_env(map_name):
    return gym.make("FrozenLake-v1", desc=MAPS[map_name], is_slippery=False)
...
def train(mode, map_name, seed, episodes=2000, frozen_sit=True):
    # Set Seed
    np.random.seed(seed)
    torch.manual_seed(seed)
    
    run_name = f"robust_{map_name}_{mode}_f{frozen_sit}_s{seed}_{int(time.time())}"
    writer = SummaryWriter(f"logs/robust_study/{run_name}")
    
    env = make_env(map_name)
    n_states = len(MAPS[map_name]) * len(MAPS[map_name][0])
    ...
    for ep in range(episodes):
        ...
        active_hier = True
        if mode == "hierarchical" and frozen_sit and ep < 500: active_hier = False

        s_oh = torch.zeros(n_states); s_oh[s] = 1.0
        if mode == "hierarchical":
            g_probs = manager.net(s_oh)
            target = Categorical(g_probs).sample() if active_hier else torch.tensor(0)
        ...
        if ep % 100 == 0:
            writer.add_scalar("charts/episodic_return", np.mean(recent_rewards), ep)
...
if __name__ == "__main__":
    ...
    train(args.mode, args.map, args.seed)
