...
class SITDirector:
    ...
    def visualize_clusters(self, ep):
        grid = np.zeros((8, 8))
        for state, cluster in self.graph_tracker.clusters.items():
            r, c = divmod(state, 8)
            grid[r, c] = cluster
        plt.imshow(grid, cmap='tab20')
        plt.title(f"SIT-Director Cluster Discovery (Ep {ep})")
        plt.savefig(f"sit_clusters_ep{ep}.png")
        plt.close()
        ...
        for ep in range(episodes):
            ...
            while not done:
                ...
                if next_cl != curr_cl or intrinsic_reward > 0 or done:
                    m_rewards.append(reward + (0.1 if intrinsic_reward > 0 else 0))
            ...
            if (ep + 1) % 100 == 0:
                n_c = self.graph_tracker.run_sit_optimization()
                print(f"Episode {ep+1} | Reward: {total_reward} | Clusters: {n_c}")
                self.visualize_clusters(ep + 1)

if __name__ == "__main__":
    env = make_env()
    agent = SITDirector(env)
    agent.train(500)
    print("SIT-Director Prototype Run Complete.")
